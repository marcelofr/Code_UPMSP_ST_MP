...
LSSolution * Destruction(LSSolution * solution, unsigned level){
    ...
    /*//Identificar a máquina que está com o makespan
    machine_max_ct = 1;
    max_ct = solution->machine_completion_time[machine_max_ct];
    machine_min_ct = 1;
    min_ct = solution->machine_completion_time[machine_min_ct];
    for(unsigned i=2;i<=Instance::num_machine ;i++ ){
        if(solution->machine_completion_time[i] > max_ct){
            machine_max_ct = i;
            max_ct = solution->machine_completion_time[machine_max_ct];
        }
        else if(solution->machine_completion_time[i] < min_ct){
            machine_min_ct = i;
            min_ct = solution->machine_completion_time[machine_min_ct];
        }
    }
    ...
    for (unsigned i=0;i < solution->scheduling[machine_max_ct].size();i++ ) {
        ...
        solution->RemovingJob(machine_max_ct, i);

    }

    for (unsigned i=0;i < solution->scheduling[machine_min_ct].size();i++ ) {
        ...
        solution->RemovingJob(machine_min_ct, i);
...
void Construction(LSSolution * partial_solution, NDSetSolution<LSSolution *> &non_dominated_set, vector<unsigned> removed_jobs){
    ...
    for(auto it: non_dominated_set_partial.set_solution){
        non_dominated_set.AddSolution(it);
    }
...
bool IntesificationArroyo(LSSolution *current_solution, NDSetSolution<LSSolution *> &non_dominated_set, unsigned level){
    ...
    *partial_solution = *current_solution;
    ...
    Construction(partial_solution, non_dominated_set, removed_jobs);
